...
if (!html.includes('id="welcomeGuideDialog"') || !html.includes('id="welcomeGuideOpen"') || !app.includes('GUIDE_INVITE_STORAGE_KEY = "astra-guide-invite-seen-v2"') || !app.includes("offerFirstVisitGuide") || app.includes('welcomeGuideDialog.addEventListener("close", rememberGuideInvite)')) throw new Error("First-visit guide invitation must reappear until an explicit choice is made");
...
if (!html.includes("Нажмите активную точку второй раз — выбор снимется") || !styles.includes("demoCursorPress 160ms") || !styles.includes(".guide-dialog .demo-loop-label, .guide-dialog .cross-demo-status { display: none; }")) throw new Error("Compact mobile guide and visible cursor press feedback are missing");
...
if (!html.includes('id="practiceModeButton"') || !html.includes('id="gradedModeButton"') || !html.includes('id="quizGradedModeButton"') || !html.includes('Контрольная: созвездия') || !html.includes('Контрольная: викторина') || !html.includes('id="testIntroDialog"') || !html.includes('id="testHud"')) throw new Error("Constellation and quiz testing interfaces are missing");
...
if (!app.includes('includeExtras: {') || !app.includes('objectIndicesForMode("practice")') || !app.includes('objectIndicesForMode("graded", true)') || !app.includes('objectIndicesForMode("quizGraded", true)') || !app.includes('quizPool(')) throw new Error("Core and extended mode pools are not isolated");
if (!html.includes('id="showAssessmentsToggle"') || !app.includes('showAssessments: Boolean') || !app.includes('function syncAssessmentModes()') || !styles.includes('.mode-switcher.has-assessments')) throw new Error("Optional assessment navigation setting is missing");
if (html.includes('class="quiz-orbit"') || styles.includes('.quiz-orbit')) throw new Error("Decorative quiz orbit must be removed");
if (!app.includes('state.quiz.transitionTimer = setTimeout(() => renderQuizQuestion(true), 850)') || !styles.includes('quizQuestionIn 160ms') || !app.includes('state.test.pendingKind === "quiz"')) throw new Error("Automatic quiz progression or graded quiz flow is missing");
...
if (html.includes('id="testButton"') || !app.includes('setModeSelection(quizTest ? "quizGraded" : "graded")') || !app.includes('setModeSelection("practice")')) throw new Error("Testing modes must use the dedicated top-level mode switcher");
