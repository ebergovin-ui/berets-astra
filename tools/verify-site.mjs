...
if (!html.includes('id="testIntegrityDialog"') || !app.includes('document.addEventListener("visibilitychange", handleTestVisibility)') || !app.includes('state.test.visibilityExits === 1') || !app.includes('resolveTestRound(false, "Повторный выход со страницы во время контрольной.", false)') || !app.includes('testIntegrityDialog.addEventListener("cancel"')) throw new Error("Testing mode visibility integrity rule is missing");
if (!app.includes('portraitPhone') || !app.includes('"160 0 680 680"')) throw new Error("Portrait mobile field viewport optimization is missing");
